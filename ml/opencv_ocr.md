...
pytesser 调用�?tesseract，因此需要安�?tesseract，安�?tesseract 需要安�?leptonica，否则编译tesseract 的时候出�?"configure: error: leptonica not found"�?
...
http://www.leptonica.org/download.html 或�?
...
最新的�?tesseract-ocr-3.02.02.tar.gz

 
下载安装 tesseract 的语言数据�?

http://code.google.com/p/tesseract-ocr/downloads/list

最新的�?tesseract-ocr-3.01.eng.tar.gz

解压tessdata目录下的文件�?个）�?"/usr/local/share/tessdata"目录�?
...
pytesser的验证码识别能力较低，只能对规规矩矩不歪不斜数字和字母验证码进行识别。测试了几个网站的验证码，显�?Empty page，看来用它来识别验证码是无望了�?
...
Traceback (most recent call last):
  ...
  File "<YOUR_HOME>/Documentos/Geek/pytesser.py", line 31, in image_to_string
    call_tesseract(scratch_image_name, scratch_text_name_root)
  File "<YOUR_HOME>/Documentos/Geek/pytesser.py", line 21, in call_tesseract
...
tesseract    图片�? 输出文件�?-l 字库文件 -psd pagesegmode 配置文件
例如�?
tesseract code.jpg result  -l chi_sim -psd 7 nobatch
-l chi_sim 表示用简体中文字库（需要下载中文字库文件，解压后，存放到tessdata目录下去,字库文件扩展名为.raineddata，简体中文字库文件名�?  chi_sim.traineddata�?
-psd 7 表示告诉tesseract code.jpg图片是一行文�? 这个参数可以减少识别错误�?  默认�?3
configfile 参数值为tessdata\configs �? tessdata\tessconfigs 目录下的文件�?
