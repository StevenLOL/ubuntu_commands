...
安装 Sun Grid Engine 需�?Java 的支持（最好是 SUN �?Java 版本），所以所有结点都需要安�?Java 包。Ubuntu �?11.10 版本不在提供 SUN 的官�?Java 版本，需要另外的源：

$ sudo apt-get install python-software-properties
$ sudo add-apt-repository ppa:ferramroberto/java
$ sudo apt-get update
$ sudo apt-get install sun-java6-jre
首先在所有结点上修改 /etc/hosts，grid00 是主控结点，grid01 �?grid02 是执行结点（我们一般把主控结点叫做 master 或�?head，执行结点叫�?client）：

$sudo vi /etc/hosts
127.0.0.1 localhost.localdomain localhost
<SERVER_IP_1> grid00.linuxidc.com grid00
<SERVER_IP_2> grid01.linuxidc.com grid01
<SERVER_IP_3> grid02.linuxidc.com grid02
在主控结点（grid00）上安装 gridengine-master 以及一些管理工�?qhost/qconf/qstat 等（包含�?gridengine-client 中）�?

$ sudo apt-get install gridengine-client gridengine-common gridengine-master
如果不习惯命令行界面，可以在主控结点上装�?qmon 图形管理工具，界面如下：

$ sudo apt-get install xserver-xorg gridengine-qmon

在所有执行结点（grid01 �?grid02）上安装 gridengine-exec�?
